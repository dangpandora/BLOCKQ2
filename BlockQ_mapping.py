...
usage = '''
      ...
      Usage: %s <fastq dir> <SE or PE> <ref> <version> <Output Prefix>>STDOUT
...
def main():
    ...
    main_py ='{}\offlineMappingV2\\analysisAndMapping.py'.format(pyfile)
    mapDir = os.path.join(fqdir,'FQMAP')
    if sys.argv[2] == 'PE':
        cmd = 'python {} {} {} mapping -m --ref {} -p '.format(main_py, fqdir, fqdir,ref)
    else:
        cmd = 'python {} {} {} mapping -m --ref {} '.format(main_py, fqdir, fqdir,ref)
    os.system(cmd)
    os.chdir(mapDir)
    for files in os.listdir(mapDir):
        if files.endswith('bam') and len(files.split('.'))==2:
            bamfile = os.path.join(mapDir,files)
            
        if files.endswith('sam'):
            samfile = os.path.join(mapDir,files)
    ...
    Qval, mapR, fov_list, cycleQ, length,GC,Err = bam_read(bamfile,samfile)
